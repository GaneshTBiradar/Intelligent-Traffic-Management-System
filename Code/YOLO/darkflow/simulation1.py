...
buttonCoods = (200, 50)
buttonRect = pygame.Rect(buttonCoods, (100, 30)) 

# Add double buffering surface
drawing_surface = pygame.Surface((1400, 800))
...
def handleButtonClick():
    global currentGreen, currentYellow, nextGreen, will_turn,ambulance_lock,ambulance_lane
    """# Close all signals
    ...
    if(ambulance_lock==0):
        ambulance_lock=1
        vehicle_type = random.randint(5,7)
        ...
        if(temp<a[0]):
            ...
            direction_number = 3
        ambulance_lane=directionNumbers[direction_number]
...
def opensignal(direction_number):
    global currentGreen, currentYellow, nextGreen, signals, ambulance_lock
    ...
    if(currentGreen==direction_number):
        ...
        #print("opensignal:",ambulance_lock)
        
        
        currentGreen=direction_number
        # Set the signal for lane 2 (index 1) to green
        #signals[direction_number].green = defaultGreen

        # Set the current and next green signal indices
        newLoop()    # set next green signal
...
speeds = {'car':3.25, 'bus':2.8, 'truck':2.8, 'rickshaw':3, 'bike':3.5, 'ambulance': 4.5, 'police_van': 3.9, 'fire_bridage': 3.7}  # average speeds of vehicles
...
vehicleTypes = {0:'car', 1:'bus', 2:'truck', 3:'rickshaw', 4:'bike', 5:'ambulance', 6:'police_van', 7:'fire_bridage' }
...
#vehicle counts
totalVehicles = [0] * noOfSignals
cVehicles = [0] * noOfSignals
...
class Vehicle(pygame.sprite.Sprite):
    def __init__(self, lane, vehicleClass, direction_number, direction, will_turn):
        ...
        self.turned = 0
        self.rotateAngle = 0
        self.flag = 1 if vehicleClass.lower() in {'ambulance', 'police_van', 'fire_bridage'} else 0
        ...
        totalVehicles[direction_number]+=1
        ...
        if(self.flag==1):
            opensignal(direction_number)

    def render(self, screen):
        screen.blit(self.currentImage, (self.x, self.y))

    def move(self):
        global ambulance_lock,ambulance_lane
        ...
        """if self.vehicleClass.lower() in {'ambulance', 'police_van', 'fire_bridage'} and self.crossed == 0:
            ambulance_lock=0
            print("am:",ambulance_lock)  """
            
        if self.crossed == 0 and (
            (ambulance_lane == 'right' and self.x + self.currentImage.get_rect().width >= stopLines[self.direction]) or
            (ambulance_lane == 'down' and self.y + self.currentImage.get_rect().height >= stopLines[self.direction]) or
            (ambulance_lane == 'up' and self.y <= stopLines[self.direction]) or
            (ambulance_lane == 'left' and self.x <= stopLines[self.direction])) and self.vehicleClass.lower() in {'ambulance', 'police_van', 'fire_bridage'}:
                ambulance_lock=0
                #print("am:",ambulance_lock)
...
def newTime():
    global noOfCars, noOfBikes, noOfBuses, noOfTrucks, noOfRickshaws, noOfLanes, noOfAmbulance, noOfFire, noOfPolice
    global carTime, busTime, truckTime, rickshawTime, bikeTime, ambulanceTime, firebTime, policeTime
    for i in range(1,3):
        for j in range(len(vehicles[directionNumbers[nextGreen]][i])):
            ...
            if(vehicle.crossed==0):
                ...
                if(vclass=='car'):
                    ...
                    noOfAmbulance += 1
                elif vclass == 'police_van':
                    noOfPolice += 1
                elif vclass == 'fire_brigade':
                    noOfFire += 1
    # print(noOfCars)
    #+(noOfAmbulance*ambulanceTime)+(noOfPolice*policeTime)+(noOfFire*firebTime)
...
#       greenTime = defaultMaximum
    ...
    noOfAmbulance, noOfFire, noOfPolice = 0,0,0
...
def newLoop():
    global currentGreen, currentYellow, nextGreen,cVehicles
    count=nextGreen
    for i in range(currentGreen+1, noOfSignals):
        if(cVehicles[i]>20):
            nextGreen=i
            break
            
    for i in range(0,currentGreen):
        if(cVehicles[i]>20):
            nextGreen=i
            break
    if(count==nextGreen):
...
class Main:
   ...
   # screen.blit(background, (0, 0))
     # Draw the button
    pygame.draw.rect(screen, (0, 128, 255), buttonRect)
    buttonText = font.render("Emergency Vehicle", True, white, (0, 128, 255))
 #   screen.blit(buttonText, (buttonCoods[0] + 10, buttonCoods[1] + 5))
    ...
    while True:
        ...
        screen.blit(background,(0,0))
        screen.blit(buttonText, (buttonCoods[0] + 10, buttonCoods[1] + 5))# display background in simulation
        ...
        crossed_vehicle_counts = [0] * noOfSignals

        # Your existing code for displaying signal timer and vehicle count
        for i in range(0, noOfSignals):  
            ...
            # Store the crossed vehicle count in the list
            crossed_vehicle_counts[i] += displayText

        
        for i in range(0, noOfSignals):
            cVehicles[i]=totalVehicles[i]-crossed_vehicle_counts[i]
            
         # display vehicle count for each lane
        totalVehicleText = font.render("Lane1 Vehicles: {}".format(cVehicles[0]), True, white, black)
        screen.blit(totalVehicleText, (10, 240))
        totalVehicleText = font.render("Lane2 Vehicles: {}".format(cVehicles[1]), True, white, black)
        screen.blit(totalVehicleText, (810, 20))
        totalVehicleText = font.render("Lane3 Vehicles: {}".format(cVehicles[2]), True, white, black)
        screen.blit(totalVehicleText, (1210, 580))
        totalVehicleText = font.render("Lane4 Vehicles: {}".format(cVehicles[3]), True, white, black)
        screen.blit(totalVehicleText, (400, 710))
